<template>
  ...
  <RadioGroup v-model="selected">
    ...
    <div class="bg-white rounded-md -space-y-px">
      ...
      >
        <div :class="[settingIdx === 0 ? 'rounded-tl-md rounded-tr-md' : '', settingIdx === timeViews.length - 1 ? 'rounded-bl-md rounded-br-md' : '', checked ? 'bg-teal-50 border-teal-200 z-10' : 'border-gray-200', 'relative border p-4 flex cursor-pointer focus:outline-none']">
          <span
          ...
          <div class="ml-3 flex flex-col">
            <RadioGroupLabel
              as="span"
              :class="[checked ? 'text-teal-900' : 'text-gray-900', 'block text-sm font-medium']"
            >
              {{ setting.name }}
            </RadioGroupLabel>
            <RadioGroupDescription
              as="span"
              :class="[checked ? 'text-teal-700' : 'text-gray-500', 'block text-sm']"
            >
              {{ setting.description }}
            </RadioGroupDescription>
          </div>
